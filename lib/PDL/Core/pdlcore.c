...
#ifdef USE_ITHREADS
static int pdl_perl_extension_magic_dup(pTHX_ MAGIC *mg, CLONE_PARAMS *param) {
   /* Do nothing, for now, because PDL cloning is handled when the pdl struct gets used
    * by a new thread.
    */
}
#else
#define pdl_perl_extension_magic_dup 0
#endif
...
#endif
#ifdef MGf_DUP
  , pdl_perl_extension_magic_dup /* dup for new threads */
#endif
#ifdef MGf_LOCAL
  , 0 /* local */
#endif
...
pdl* pdl_SvPDLV ( SV* sv ) {
   ...
   if ( !SvROK(sv) ) {
      ...
      PDL_Anyval data;
      ANYVAL_FROM_SV(data, sv, TRUE, -1, TRUE);
